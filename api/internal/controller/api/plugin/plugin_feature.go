...
type PluginFeatureController struct {
	*controller.BaseController
}

func init() {
	c := &PluginFeatureController{
		BaseController: controller.GetBaseController(),
	}
	c.RegisterApi("POST", "/plugin_feature/page", c.Page, "基础表", "插件功能", "查看列表")
	c.RegisterApi("POST", "/plugin_feature/save", c.Save, "基础表", "插件功能", "新增/编辑")
	c.RegisterApi("POST", "/plugin_feature/detail", c.Detail, "基础表", "插件功能", "查看详情")
	c.RegisterApi("POST", "/plugin_feature/delates", c.Deletes, "基础表", "插件功能", "删除")
	c.RegisterApi("POST", "/plugin_feature/import", c.ImportXlsx, "基础表", "插件功能", "导入")
	c.RegisterApi("POST", "/plugin_feature/export", c.ExportXlsx, "基础表", "插件功能", "导出")
}
func (c *PluginFeatureController) Page(ctx *gin.Context) {
	...
	var total int64
	err = db.Model(&plugin.PluginFeature{}).Count(&total).Error
	if err != nil {
		c.ErrMsg(ctx, err)
		return
	}
	...
	var datas []plugin.PluginFeature
...
func (c *PluginFeatureController) pageParam(param controller.PageParam) (*gorm.DB, error) {
	db := c.Database().Read()
	if param.ParamString("keyword") != "" {
		db = db.Where("plugin_code like ?", "%"+param.ParamString("keyword")+"%")
	}
...
func (c *PluginFeatureController) Save(ctx *gin.Context) {
	var data plugin.PluginFeature
...
func (c *PluginFeatureController) Detail(ctx *gin.Context) {
	var data plugin.PluginFeature
	if err := ctx.ShouldBindBodyWithJSON(&data); err != nil {
		c.ErrMsg(ctx, err)
		return
...
func (c *PluginFeatureController) Deletes(ctx *gin.Context) {
	var ids []int
	...
	db := c.Database().Write()
	err := db.Model(&plugin.PluginFeature{}).Delete("id in ?", ids).Error
	if err != nil {
		c.ErrMsg(ctx, err)
		return
	}
...
func (c *PluginFeatureController) ImportXlsx(ctx *gin.Context) {
	...
	headers := []util.ExportField{
		{FieldName: "PluginCode", CnName: c.GetText(ctx, "model.plugin_feature.plugin_code")},
		{FieldName: "Enabled", CnName: c.GetText(ctx, "model.plugin_feature.enabled")},
		{FieldName: "Content", CnName: c.GetText(ctx, "model.plugin_feature.content")},
	...
	err = c.Database().Transaction(func(tx *gorm.DB) error {
		for _, v := range result {
			...
			if v["ID"] == nil {
				...
				tx2 = tx.Model(&plugin.PluginFeature{}).Create(&v)
				if tx2.RowsAffected > 0 {
					create++
				}
			} else {
				v["UpdatedAt"] = util.UnixMilliseconds()
				tx2 = tx.Model(&plugin.PluginFeature{}).Where("id = ?", v["ID"]).Save(&v)
				if tx2.RowsAffected > 0 {
					update++
				}
			}
...
func (c *PluginFeatureController) ExportXlsx(ctx *gin.Context) {
	param := c.PageParam(ctx)
	c.exportXlsx(param, ctx)
}

func (c *PluginFeatureController) exportXlsx(param controller.PageParam,ctx *gin.Context) {
	ctx.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	fileName := fmt.Sprintf("%s%s.xlsx", c.GetText(ctx, "model.plugin_feature.model"), c.GetText(ctx, "common.export"))
	...
	headers := []util.ExportField{
		{FieldName: "PluginCode", CnName: c.GetText(ctx, "model.plugin_feature.plugin_code"), FieldType: "input"},
		{FieldName: "Enabled", CnName: c.GetText(ctx, "model.plugin_feature.enabled"), FieldType: "select", Selects: map[string]string{
			"0": c.GetText(ctx, "model.plugin_feature.enabled_select_0"),
			"1": c.GetText(ctx, "model.plugin_feature.enabled_select_1"),
			}},
		{FieldName: "Content", CnName: c.GetText(ctx, "model.plugin_feature.content"), FieldType: "input"},
	...
	if param.ParamBool("isTemplate") {
		...
		pageSize := 1000
		page := 1
		err = util.ExportXlsx(headers, ctx.Writer, func() ([]plugin.PluginFeature, bool, error) {
			var datas []plugin.PluginFeature
