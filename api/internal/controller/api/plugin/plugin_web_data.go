...
type PluginWebDataController struct {
	*controller.BaseController
}

func init() {
	c := &PluginWebDataController{
		BaseController: controller.GetBaseController(),
	}
	c.RegisterApi("POST", "/plugin_web_data/page", c.Page, "基础表", "插件扩展数据", "查看列表")
	c.RegisterApi("POST", "/plugin_web_data/save", c.Save, "基础表", "插件扩展数据", "新增/编辑")
	c.RegisterApi("POST", "/plugin_web_data/detail", c.Detail, "基础表", "插件扩展数据", "查看详情")
	c.RegisterApi("POST", "/plugin_web_data/delates", c.Deletes, "基础表", "插件扩展数据", "删除")
	c.RegisterApi("POST", "/plugin_web_data/import", c.ImportXlsx, "基础表", "插件扩展数据", "导入")
	c.RegisterApi("POST", "/plugin_web_data/export", c.ExportXlsx, "基础表", "插件扩展数据", "导出")
}
func (c *PluginWebDataController) Page(ctx *gin.Context) {
	...
	var total int64
	err = db.Model(&plugin.PluginWebData{}).Count(&total).Error
	if err != nil {
		c.ErrMsg(ctx, err)
		return
	}
	...
	var datas []plugin.PluginWebData
...
func (c *PluginWebDataController) pageParam(param controller.PageParam) (*gorm.DB, error) {
	db := c.Database().Read()
	if param.ParamString("keyword") != "" {
		db = db.Where("code like ?", "%"+param.ParamString("keyword")+"%")
	}
...
func (c *PluginWebDataController) Save(ctx *gin.Context) {
...
func (c *PluginWebDataController) Detail(ctx *gin.Context) {
	var data plugin.PluginWebData
	if err := ctx.ShouldBindBodyWithJSON(&data); err != nil {
		c.ErrMsg(ctx, err)
		return
...
func (c *PluginWebDataController) Deletes(ctx *gin.Context) {
	var ids []int
	...
	db := c.Database().Write()
	err := db.Model(&plugin.PluginWebData{}).Delete("id in ?", ids).Error
	if err != nil {
		c.ErrMsg(ctx, err)
		return
	}
...
func (c *PluginWebDataController) ImportXlsx(ctx *gin.Context) {
	...
	headers := []util.ExportField{
		{FieldName: "Code", CnName: c.GetText(ctx, "model.plugin_web_data.code")},
		{FieldName: "UserID", CnName: c.GetText(ctx, "model.plugin_web_data.user_id")},
		{FieldName: "DataKey", CnName: c.GetText(ctx, "model.plugin_web_data.data_key")},
		{FieldName: "DataValue", CnName: c.GetText(ctx, "model.plugin_web_data.data_value")},
	...
	err = c.Database().Transaction(func(tx *gorm.DB) error {
		for _, v := range result {
			...
			if v["ID"] == nil {
				...
				tx2 = tx.Model(&plugin.PluginWebData{}).Create(&v)
				if tx2.RowsAffected > 0 {
					create++
				}
			} else {
				v["UpdatedAt"] = util.UnixMilliseconds()
				tx2 = tx.Model(&plugin.PluginWebData{}).Where("id = ?", v["ID"]).Save(&v)
				if tx2.RowsAffected > 0 {
					update++
				}
			}
...
func (c *PluginWebDataController) ExportXlsx(ctx *gin.Context) {
	param := c.PageParam(ctx)
	c.exportXlsx(param, ctx)
}

func (c *PluginWebDataController) exportXlsx(param controller.PageParam,ctx *gin.Context) {
	ctx.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	fileName := fmt.Sprintf("%s%s.xlsx", c.GetText(ctx, "model.plugin_web_data.model"), c.GetText(ctx, "common.export"))
	...
	headers := []util.ExportField{
		{FieldName: "Code", CnName: c.GetText(ctx, "model.plugin_web_data.code"), FieldType: "input"},
		{FieldName: "UserID", CnName: c.GetText(ctx, "model.plugin_web_data.user_id"), FieldType: "input"},
		{FieldName: "DataKey", CnName: c.GetText(ctx, "model.plugin_web_data.data_key"), FieldType: "input"},
		{FieldName: "DataValue", CnName: c.GetText(ctx, "model.plugin_web_data.data_value"), FieldType: "textarea"},
	...
	if param.ParamBool("isTemplate") {
		...
		pageSize := 1000
		page := 1
		err = util.ExportXlsx(headers, ctx.Writer, func() ([]plugin.PluginWebData, bool, error) {
			var datas []plugin.PluginWebData
