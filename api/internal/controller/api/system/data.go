...
func init() {
	...
	c.RegisterApi("POST", "/data/page", c.Page, "基础表", "数据", "查看列表")
	c.RegisterApi("POST", "/data/save", c.Save, "基础表", "数据", "新增/编辑")
	c.RegisterApi("POST", "/data/detail", c.Detail, "基础表", "数据", "查看详情")
	c.RegisterApi("POST", "/data/delates", c.Deletes, "基础表", "数据", "删除")
	c.RegisterApi("POST", "/data/import", c.ImportXlsx, "基础表", "数据", "导入")
	c.RegisterApi("POST", "/data/export", c.ExportXlsx, "基础表", "数据", "导出")
}
func (c *DataController) Page(ctx *gin.Context) {
	...
	var total int64
	err = db.Model(&system.Data{}).Count(&total).Error
	if err != nil {
		c.ErrMsg(ctx, err)
		return
	}
...
func (c *DataController) pageParam(param controller.PageParam) (*gorm.DB, error) {
	db := c.Database().Read()
	if param.ParamString("keyword") != "" {
		db = db.Where("data_key like ?", "%"+param.ParamString("keyword")+"%")
	}
...
func (c *DataController) Save(ctx *gin.Context) {
...
func (c *DataController) Detail(ctx *gin.Context) {
	var data system.Data
	if err := ctx.ShouldBindBodyWithJSON(&data); err != nil {
		c.ErrMsg(ctx, err)
		return
...
func (c *DataController) Deletes(ctx *gin.Context) {
	var ids []int
	...
	db := c.Database().Write()
	err := db.Model(&system.Data{}).Delete("id in ?", ids).Error
	if err != nil {
		c.ErrMsg(ctx, err)
		return
	}
...
func (c *DataController) ImportXlsx(ctx *gin.Context) {
	...
	headers := []util.ExportField{
		{FieldName: "DataKey", CnName: c.GetText(ctx, "model.data.data_key")},
		{FieldName: "DataValue", CnName: c.GetText(ctx, "model.data.data_value")},
	...
	err = c.Database().Transaction(func(tx *gorm.DB) error {
		for _, v := range result {
			...
			if v["ID"] == nil {
				...
				tx2 = tx.Model(&system.Data{}).Create(&v)
				if tx2.RowsAffected > 0 {
					create++
				}
			} else {
				v["UpdatedAt"] = util.UnixMilliseconds()
				tx2 = tx.Model(&system.Data{}).Where("id = ?", v["ID"]).Save(&v)
				if tx2.RowsAffected > 0 {
					update++
				}
			}
...
func (c *DataController) ExportXlsx(ctx *gin.Context) {
	param := c.PageParam(ctx)
	c.exportXlsx(param, ctx)
}

func (c *DataController) exportXlsx(param controller.PageParam,ctx *gin.Context) {
	ctx.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	fileName := fmt.Sprintf("%s%s.xlsx", c.GetText(ctx, "model.data.model"), c.GetText(ctx, "common.export"))
	...
	headers := []util.ExportField{
		{FieldName: "DataKey", CnName: c.GetText(ctx, "model.data.data_key"), FieldType: "input"},
		{FieldName: "DataValue", CnName: c.GetText(ctx, "model.data.data_value"), FieldType: "textarea"},
	...
	if param.ParamBool("isTemplate") {
		...
		pageSize := 1000
		page := 1
		err = util.ExportXlsx(headers, ctx.Writer, func() ([]system.Data, bool, error) {
			var datas []system.Data
