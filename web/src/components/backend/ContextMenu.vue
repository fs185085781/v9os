...
<template>
  <div
    v-if="cm.state.show"
    class="backend-context-menu fixed z-9999 min-w-52 py-1 text-sm user-color-ftext user-color-fbg border user-color-border shadow-lg user-rounded-2"
  ...
  >
    <template v-for="item in cm.state.items" :key="item.key">
      ...
      <div
        v-else
        class="backend-context-item relative flex items-center justify-between h-8 px-3 mx-1 cursor-default user-rounded-1.5"
        :class="{
      ...
      >
        ...
        <div
          ...
          class="backend-context-submenu absolute hidden min-w-52 py-1 user-color-ftext user-color-fbg border user-color-border shadow-lg user-rounded-2 max-h-[calc(100vh-16px)] overflow-y-auto"
          :style="{ top: cm.state.submenuTopMap[item.key] || '-4px' }"
        ...
        >
          <template v-for="child in item.children" :key="child.key">
            ...
            <div
              v-else
              class="flex items-center h-8 px-3 mx-1 cursor-default user-rounded-1.5"
              :class="{
...
<style scoped>
.backend-context-item:hover > .backend-context-submenu {
  display: block;
}
</style>
