...
const downloadTemplate = async () => {
  const data = await postBlob("plugin_table_expand_task", "import", { isTemplate: true })
  if (data == null) {
    return
  }
...
const handleFileUpload = async (event) => {
  ...
  let msg = "";
  let flag = await postData("plugin_table_expand_task", "import", formData, '', function (res) {
    msg = res;
  })
  if (flag) {
    data.title = $tc("common.all.importSuccess")
    data.type = "success"
    data.content = computed(() => { return msg })
...
<template>
  ...
  <input ref="fileInput" type="file" class="hidden" accept=".xlsx" @change="handleFileUpload">
</template>