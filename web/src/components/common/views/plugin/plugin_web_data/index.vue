...
const columnFn = () =>[
    ...
    {
        title: $t("model.plugin_web_data.code"),
        key: 'Code',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        title: $t("model.plugin_web_data.user_id"),
        key: 'UserID',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        title: $t("model.plugin_web_data.data_key"),
        key: 'DataKey',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        title: $t("model.plugin_web_data.data_value"),
        key: 'DataValue',
        sorter: true,
        resizable: true,
        ellipsis: true,
    ...
    },
    {
        ...
        render(row) {
            const buttons = [];
            if (checkAuth("/api/plugin_web_data/save")) {
                buttons.push(h(
                    NButton,
                    {
                        size: 'tiny',
            ...
            if (checkAuth("/api/plugin_web_data/detail")) {
                buttons.push(h(
                    NButton,
                    {
                        class: "m-l-1",
            ...
            if (checkAuth("/api/plugin_web_data/delates")) {
                buttons.push(h(
                    NButton,
                    {
                        class: "m-l-1",
...
const toEdit = async (tmp) => {
    let row = null;
    if(tmp.ID){
        row = await postData("plugin_web_data","detail",{ID:tmp.ID})
    }else{
        row = cloneDeep(tmp)
    }
    ws.addWindow({
        ...
        height: 350,
        title: computed(() => {
            return row.ID?$t("common.all.edit2",{entity:$t("model.plugin_web_data.model")}):$t("common.all.add2",{entity:$t("model.plugin_web_data.model")})
        }),
        component: EditCom,
        data: row
    },winId)
...
const toDetail = async (row) => {
    const res = await postData("plugin_web_data","detail",{ID:row.ID})
    if(res){
        ws.addWindow({
            width: 500,
            height: 350,
            title: computed(() => {
            return $t("common.all.detail2",{entity:$t("model.plugin_web_data.model")})
            }),
            component: DetailCom,
            data: res
        },winId)
...
const toDelate = async (row) => {
    ...
    const res = await postData("plugin_web_data","delates",[row.ID])
    if(res){
        query()
    }
}
const toDelates = async () => {
    ...
    const res = await postData("plugin_web_data","delates",selectedKeys.value)
    if(res){
        query()
    }
}
...
const exportXlsx = async () => {
    const data = await postBlob("plugin_web_data","export",param,"okerr")
    if(data == null){
        return
    }
...
const query = async () => {
    ...
    const res = await postData("plugin_web_data","page",param)
    if (res) {
        data.value = res.data
...
useEventBus("plugin_web_data-edit-over",(msg) => {
    query()
})
...
<template>
    <div v-if="!checkAuth('/api/plugin_web_data/page')" class="flex items-center justify-center h-full">
        <div>{{ $t("common.all.pagenoauth") }}</div>
    </div>
    <div class="h-full" v-auth="'/api/plugin_web_data/page'">
        <n-card class="h-full rounded-0" :bordered="false">
            <div class="h-full">
                <n-flex inline>
                    <div class="inline-flex">
                            <div class="w-32 line-height-7">{{$t("model.plugin_web_data.code")}}</div>
                            <n-input v-model:value="param.keyword" :placeholder="$t('common.all.searchKeyword',{keyword:$t('model.plugin_web_data.code')})" size="small" />
                    </div>
                    <n-button type="primary" size="small" @click="search()">
                        {{ $t("common.all.search") }}
                    </n-button>
                    <n-button v-auth="'/api/plugin_web_data/save'" type="primary" size="small" @click="toEdit({})">
                        {{ $t("common.all.add") }}
                    </n-button>
                    <n-button v-auth="'/api/plugin_web_data/delates'" type="error" size="small" v-if="selectedKeys.length > 0" @click="toDelates()">
                        {{ $t("common.all.delete") }}({{ selectedKeys.length }})
                    </n-button>
                    <n-button v-auth="'/api/plugin_web_data/import'" type="default" size="small" @click="importXlsx()">
                        {{ $t("common.all.import") }}
                    </n-button>
                    <n-button v-auth="'/api/plugin_web_data/export'" type="primary" size="small" @click="exportXlsx()">
                        {{ $t("common.all.export") }}
                    </n-button>
                </n-flex>
                <n-data-table
