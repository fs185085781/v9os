...
const levelOptionMap = computed(() => {
    return {
    "debug": $t("model.log.level_select_debug"),
    "error": $t("model.log.level_select_error"),
    "info": $t("model.log.level_select_info"),
    "warn": $t("model.log.level_select_warn"),
    }
})
const columnFn = () =>[
    {
    ...
    {
        title: $t("model.log.level"),
        key: 'Level',
        sorter: true,
        resizable: true,
        ellipsis: true,
        minWidth: 120,render(row) {
            return levelOptionMap.value[row.Level]
        }
    },{
        title: $t("model.log.msg"),
        key: 'Msg',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        ,render(row) {
            return new Date(row.Time).format()
        }
    },{
        title: $t("model.log.text"),
        key: 'Text',
        sorter: true,
        resizable: true,
        ellipsis: true,
...
const toEdit = async (tmp) => {
    let row = null;
    if(tmp.ID){
        row = await postData("log","detail",{ID:tmp.ID})
    }else{
        row = cloneDeep(tmp)
    }
    ws.addWindow({
        ...
        height: 350,
        title: computed(() => {
            return row.ID?$t("common.all.edit2",{entity:$t("model.log.model")}):$t("common.all.add2",{entity:$t("model.log.model")})
        }),
        component: EditCom,
        data: row
    },winId)
...
const toDetail = async (row) => {
    const res = await postData("log","detail",{ID:row.ID})
    if(res){
        ws.addWindow({
            width: 500,
            height: 350,
            title: computed(() => {
            return $t("common.all.detail2",{entity:$t("model.log.model")})
            }),
            component: DetailCom,
            data: res
        },winId)
...
const toDelate = async (row) => {
    ...
    const res = await postData("log","delates",[row.ID])
    if(res){
        query()
    }
}
const toDelates = async () => {
    ...
    const res = await postData("log","delates",selectedKeys.value)
    if(res){
        query()
    }
}
...
const exportXlsx = async () => {
    const data = await postBlob("log","export",param,"okerr")
    if(data == null){
        return
    }
...
const query = async () => {
    ...
    const res = await postData("log","page",param)
    if (res) {
        data.value = res.data
...
<template>
    <div v-if="!checkAuth('/api/log/page')" class="flex items-center justify-center h-full">
        <div>{{ $t("common.all.pagenoauth") }}</div>
    </div>
    <div class="h-full" v-auth="'/api/log/page'">
        <n-card class="h-full rounded-0" :bordered="false">
            <div class="h-full">
                <n-flex inline>
                    <div class="inline-flex">
                            <div class="w-32 line-height-7">{{$t("model.log.msg")}}</div>
                            <n-input v-model:value="param.keyword" :placeholder="$t('common.all.searchKeyword',{keyword:$t('model.log.msg')})" size="small" />
                    </div>
                    <n-button type="primary" size="small" @click="search()">
                        {{ $t("common.all.search") }}
                    </n-button>
                    <n-button v-auth="'/api/log/save'" type="primary" size="small" @click="toEdit({})">
                        {{ $t("common.all.add") }}
                    </n-button>
                    <n-button v-auth="'/api/log/delates'" type="error" size="small" v-if="selectedKeys.length > 0" @click="toDelates()">
                        {{ $t("common.all.delete") }}({{ selectedKeys.length }})
                    </n-button>
                    <n-button v-auth="'/api/log/import'" type="default" size="small" @click="importXlsx()">
                        {{ $t("common.all.import") }}
                    </n-button>
                    <n-button v-auth="'/api/log/export'" type="primary" size="small" @click="exportXlsx()">
                        {{ $t("common.all.export") }}
                    </n-button>
                </n-flex>
                <n-data-table
