...
const toEdit = async (tmp) => {
    let row = null;
    if(tmp.ID){
        row = await postData("data","detail",{ID:tmp.ID})
    }else{
        row = cloneDeep(tmp)
    }
    ws.addWindow({
        ...
        height: 350,
        title: computed(() => {
            return row.ID?$t("common.all.edit2",{entity:$t("model.data.model")}):$t("common.all.add2",{entity:$t("model.data.model")})
        }),
        component: EditCom,
        data: row
    },winId)
...
const toDetail = async (row) => {
    const res = await postData("data","detail",{ID:row.ID})
    if(res){
        ws.addWindow({
            width: 500,
            height: 350,
            title: computed(() => {
            return $t("common.all.detail2",{entity:$t("model.data.model")})
            }),
            component: DetailCom,
            data: res
        },winId)
...
const toDelate = async (row) => {
    ...
    const res = await postData("data","delates",[row.ID])
    if(res){
        query()
    }
}
const toDelates = async () => {
    ...
    const res = await postData("data","delates",selectedKeys.value)
    if(res){
        query()
    }
}
...
const exportXlsx = async () => {
    const data = await postBlob("data","export",param,"okerr")
    if(data == null){
        return
    }
...
const query = async () => {
    ...
    const res = await postData("data","page",param)
    if (res) {
        data.value = res.data
...
useEventBus("data-edit-over",(msg) => {
    query()
})
...
<template>
    <div v-if="!checkAuth('/api/data/page')" class="flex items-center justify-center h-full">
        <div>{{ $t("common.all.pagenoauth") }}</div>
    </div>
    <div class="h-full" v-auth="'/api/data/page'">
        <n-card class="h-full rounded-0" :bordered="false">
            <div class="h-full">
                <n-flex inline>
                    <div class="inline-flex">
                            <div class="w-32 line-height-7">{{$t("model.data.data_key")}}</div>
                            <n-input v-model:value="param.keyword" :placeholder="$t('common.all.searchKeyword',{keyword:$t('model.data.data_key')})" size="small" />
                    </div>
                    <n-button type="primary" size="small" @click="search()">
                        {{ $t("common.all.search") }}
                    </n-button>
                    <n-button v-auth="'/api/data/save'" type="primary" size="small" @click="toEdit({})">
                        {{ $t("common.all.add") }}
                    </n-button>
                    <n-button v-auth="'/api/data/delates'" type="error" size="small" v-if="selectedKeys.length > 0" @click="toDelates()">
                        {{ $t("common.all.delete") }}({{ selectedKeys.length }})
                    </n-button>
                    <n-button v-auth="'/api/data/import'" type="default" size="small" @click="importXlsx()">
                        {{ $t("common.all.import") }}
                    </n-button>
                    <n-button v-auth="'/api/data/export'" type="primary" size="small" @click="exportXlsx()">
                        {{ $t("common.all.export") }}
                    </n-button>
                </n-flex>
                <n-data-table
