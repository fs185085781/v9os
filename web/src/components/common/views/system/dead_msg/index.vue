...
const stypeOptionMap = computed(() => {
    return {
    "1": $t("model.dead_msg.stype_select_1"),
    "2": $t("model.dead_msg.stype_select_2"),
    "3": $t("model.dead_msg.stype_select_3"),
    "4": $t("model.dead_msg.stype_select_4"),
    }
})
const columnFn = () =>[
    {
    ...
    },
    {
        ...
        title: $t("model.dead_msg.stype"),
        key: 'Stype',
        sorter: true,
        resizable: true,
        ellipsis: true,
        minWidth: 120,render(row) {
            return stypeOptionMap.value[row.Stype]
        }
    },{
        title: $t("model.dead_msg.url"),
        key: 'Url',
        sorter: true,
        resizable: true,
        ellipsis: true,
    ...
    },
    {
        ...
        render(row) {
            const buttons = [];
            if (checkAuth("/api/dead_msg/save")) {
                buttons.push(h(
                    NButton,
                    {
                        size: 'tiny',
            ...
            if (checkAuth("/api/dead_msg/detail")) {
                buttons.push(h(
                    NButton,
                    {
                        class: "m-l-1",
            ...
            if (checkAuth("/api/dead_msg/delates")) {
                buttons.push(h(
                    NButton,
                    {
                        class: "m-l-1",
...
const toEdit = async (tmp) => {
    let row = null;
    if(tmp.ID){
        row = await postData("dead_msg","detail",{ID:tmp.ID})
    }else{
        row = cloneDeep(tmp)
    }
    ws.addWindow({
        ...
        height: 350,
        title: computed(() => {
            return row.ID?$t("common.all.edit2",{entity:$t("model.dead_msg.model")}):$t("common.all.add2",{entity:$t("model.dead_msg.model")})
        }),
        component: EditCom,
        data: row
    },winId)
...
const toDetail = async (row) => {
    const res = await postData("dead_msg","detail",{ID:row.ID})
    if(res){
        ws.addWindow({
            width: 500,
            height: 350,
            title: computed(() => {
            return $t("common.all.detail2",{entity:$t("model.dead_msg.model")})
            }),
            component: DetailCom,
            data: res
        },winId)
...
const toDelate = async (row) => {
    ...
    const res = await postData("dead_msg","delates",[row.ID])
    if(res){
        query()
    }
}
const toDelates = async () => {
    ...
    const res = await postData("dead_msg","delates",selectedKeys.value)
    if(res){
        query()
    }
}
...
const exportXlsx = async () => {
    const data = await postBlob("dead_msg","export",param,"okerr")
    if(data == null){
        return
    }
...
const query = async () => {
    ...
    const res = await postData("dead_msg","page",param)
    if (res) {
        data.value = res.data
...
useEventBus("dead_msg-edit-over",(msg) => {
    query()
})
...
<template>
    <div v-if="!checkAuth('/api/dead_msg/page')" class="flex items-center justify-center h-full">
        <div>{{ $t("common.all.pagenoauth") }}</div>
    </div>
    <div class="h-full" v-auth="'/api/dead_msg/page'">
        <n-card class="h-full rounded-0" :bordered="false">
            <div class="h-full">
                <n-flex inline>
                    <div class="inline-flex">
                            <div class="w-32 line-height-7">{{$t("model.dead_msg.plugin")}}</div>
                            <n-input v-model:value="param.keyword" :placeholder="$t('common.all.searchKeyword',{keyword:$t('model.dead_msg.plugin')})" size="small" />
                    </div>
                    <n-button type="primary" size="small" @click="search()">
                        {{ $t("common.all.search") }}
                    </n-button>
                    <n-button v-auth="'/api/dead_msg/save'" type="primary" size="small" @click="toEdit({})">
                        {{ $t("common.all.add") }}
                    </n-button>
                    <n-button v-auth="'/api/dead_msg/delates'" type="error" size="small" v-if="selectedKeys.length > 0" @click="toDelates()">
                        {{ $t("common.all.delete") }}({{ selectedKeys.length }})
                    </n-button>
                    <n-button v-auth="'/api/dead_msg/import'" type="default" size="small" @click="importXlsx()">
                        {{ $t("common.all.import") }}
                    </n-button>
                    <n-button v-auth="'/api/dead_msg/export'" type="primary" size="small" @click="exportXlsx()">
                        {{ $t("common.all.export") }}
                    </n-button>
                </n-flex>
                <n-data-table
