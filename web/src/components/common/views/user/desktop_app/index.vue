...
const app_typeOptionMap = computed(() => {
    return {
    "iframe": $t("model.desktop_app.app_type_select_iframe"),
    "plugin": $t("model.desktop_app.app_type_select_plugin"),
    "system": $t("model.desktop_app.app_type_select_system"),
    }
})
const columnFn = () =>[
    {
    ...
    {
        title: $t("model.desktop_app.user_id"),
        key: 'UserID',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        title: $t("model.desktop_app.icon"),
        key: 'Icon',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        title: $t("model.desktop_app.title"),
        key: 'Title',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        title: $t("model.desktop_app.app_type"),
        key: 'AppType',
        sorter: true,
        resizable: true,
        ellipsis: true,
        minWidth: 120,render(row) {
            return app_typeOptionMap.value[row.AppType]
        }
    },{
        title: $t("model.desktop_app.code"),
        key: 'Code',
        sorter: true,
        resizable: true,
        ellipsis: true,
        ...
        title: $t("model.desktop_app.sort"),
        key: 'Sort',
        sorter: true,
        resizable: true,
        ellipsis: true,
    ...
    },
    {
        ...
        render(row) {
            const buttons = [];
            if (checkAuth("/api/desktop_app/save")) {
                buttons.push(h(
                    NButton,
                    {
                        size: 'tiny',
            ...
            if (checkAuth("/api/desktop_app/detail")) {
                buttons.push(h(
                    NButton,
                    {
                        class: "m-l-1",
            ...
            if (checkAuth("/api/desktop_app/delates")) {
                buttons.push(h(
                    NButton,
                    {
                        class: "m-l-1",
...
const toEdit = async (tmp) => {
    let row = null;
    if(tmp.ID){
        row = await postData("desktop_app","detail",{ID:tmp.ID})
    }else{
        row = cloneDeep(tmp)
    }
    ws.addWindow({
        ...
        height: 350,
        title: computed(() => {
            return row.ID?$t("common.all.edit2",{entity:$t("model.desktop_app.model")}):$t("common.all.add2",{entity:$t("model.desktop_app.model")})
        }),
        component: EditCom,
        data: row
    },winId)
...
const toDetail = async (row) => {
    const res = await postData("desktop_app","detail",{ID:row.ID})
    if(res){
        ws.addWindow({
            width: 500,
            height: 350,
            title: computed(() => {
            return $t("common.all.detail2",{entity:$t("model.desktop_app.model")})
            }),
            component: DetailCom,
            data: res
        },winId)
...
const toDelate = async (row) => {
    ...
    const res = await postData("desktop_app","delates",[row.ID])
    if(res){
        query()
    }
}
const toDelates = async () => {
    ...
    const res = await postData("desktop_app","delates",selectedKeys.value)
    if(res){
        query()
    }
}
...
const exportXlsx = async () => {
    const data = await postBlob("desktop_app","export",param,"okerr")
    if(data == null){
        return
    }
...
const query = async () => {
    ...
    const res = await postData("desktop_app","page",param)
    if (res) {
        data.value = res.data
...
useEventBus("desktop_app-edit-over",(msg) => {
    query()
})
...
<template>
    <div v-if="!checkAuth('/api/desktop_app/page')" class="flex items-center justify-center h-full">
        <div>{{ $t("common.all.pagenoauth") }}</div>
    </div>
    <div class="h-full" v-auth="'/api/desktop_app/page'">
        <n-card class="h-full rounded-0" :bordered="false">
            <div class="h-full">
                <n-flex inline>
                    <div class="inline-flex">
                            <div class="w-32 line-height-7">{{$t("model.desktop_app.icon")}}</div>
                            <n-input v-model:value="param.keyword" :placeholder="$t('common.all.searchKeyword',{keyword:$t('model.desktop_app.icon')})" size="small" />
                    </div>
                    <n-button type="primary" size="small" @click="search()">
                        {{ $t("common.all.search") }}
                    </n-button>
                    <n-button v-auth="'/api/desktop_app/save'" type="primary" size="small" @click="toEdit({})">
                        {{ $t("common.all.add") }}
                    </n-button>
                    <n-button v-auth="'/api/desktop_app/delates'" type="error" size="small" v-if="selectedKeys.length > 0" @click="toDelates()">
                        {{ $t("common.all.delete") }}({{ selectedKeys.length }})
                    </n-button>
                    <n-button v-auth="'/api/desktop_app/import'" type="default" size="small" @click="importXlsx()">
                        {{ $t("common.all.import") }}
                    </n-button>
                    <n-button v-auth="'/api/desktop_app/export'" type="primary" size="small" @click="exportXlsx()">
                        {{ $t("common.all.export") }}
                    </n-button>
                </n-flex>
                <n-data-table
