...
$contextMenu.register("macos.desktop.shortcut", appMenuResolver);
onUnmounted(() => {
  $contextMenu.unregister("macos.desktop.shortcut", appMenuResolver);
});
</script>

<template>
  <div class="macos-desktop-icons fixed left-0 top-7 bottom-18 z-1 p-3 flex flex-col flex-wrap content-start gap-y-3">
    <button
      v-for="app in visibleApps"
      :key="app.code"
      class="macos-desktop-icon"
      @dblclick="openApp(app)"
      @click.stop
      v-context-menu="appContext(app)"
    >
      <IconView :icon="app.icon" :alt="appName(app)" :size="48" icon-class="w-12 h-12 object-contain mx-auto" />
